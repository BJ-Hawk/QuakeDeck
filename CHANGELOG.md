...
- Excludes routine P2PQuake code-555 peer-count broadcasts from delivery diagnostics and retains diagnostics until both safety thresholds have been reached: at least 200 entries and at least 2 MB, whichever comes later.
- Adds opt-in P2PQuake code-9611 felt-report counters under Data & connection. Informative aggregates attach only to one matching active EEW or confirmed earthquake; singleton zero-confidence reports remain diagnostic evidence and never create a card. A later ordinary report amends the EEW incident with official hypocentre, magnitude, depth, intensity, and observations while retaining the cumulative P2PQuake counter.
- Associates informative P2PQuake felt-report aggregates with the nearest confirmed earthquake across the realistic post-origin reporting delay, and lets the baseline P2PQuake stream attach the same evidence to an active DM-D.S.S EEW. Unmatched aggregates remain diagnostics-only and never create a standalone felt event.
- Saves incoming EEW forecasts and attached cumulative P2PQuake felt-report aggregates in the existing historical-report replay sequence. The current Previous/Next replay controls now step through those received states without adding controls or animating P/S waves in historical mode.
- Stores accepted, deduplicated DM-D.S.S `VXSE44`/`VXSE45` Forecast revisions in the same source-neutral EEW replay-frame format as P2PQuake code `556` Warnings. The shared frame path preserves source, receipt ordering, alert level, revision, forecast areas, and intensity state, so a future DM-D.S.S Warning source can replace `556` without changing the archive schema or historical viewer.
- Adds separately labelled local DM-D.S.S intensity estimates for bulletins whose official regional list is empty. The ignored FULL engine now follows JMA's published Mj/Mw, finite-source distance, PGV, AVS30 amplification, and instrumental-intensity method; aggregates 4,243 valid bundled station meshes into all 188 reporting areas; marks results below the published validation range; suppresses assumed-source PLUM and deeper-than-150-km source-plus-magnitude calculations; and uses the result for map shading and selected-location notification decisions only when official values are unavailable. P/S rings and arrivals now interpolate the official JMA2001 table instead of fixed 7/4 km/s velocities. Tracked DM-D.S.S plumbing preserves source accuracy, magnitude unit, region code, PLUM, and warning metadata; cold-start restoration, diagnostics, and Sandbox support the separate result. The NIED J-SHIS V4 resource is a reproducibly generated station-only derivative with 83 missing/uncovered coordinates and 34 invalid zero-AVS records handled explicitly. FULL and forced-LITE compile/unit-test validation pass without building an APK.
- Extends those estimates into an official-authoritative hybrid forecast. Every received JMA region remains untouched, while the FULL engine fills only missing Shindo 1–3 regions, calibrates its continuous field against matching official regional categories plus the nationwide maximum, and constrains official omissions below Shindo 4. Selected-location filtering can use the local destination estimate when another area has an official value; Forecast/Warning identity and escalation remain exclusively provider-controlled. The expanded list and diagnostics disclose official anchors, local supplements, and any omission ceilings.
...
- Distinguishes active Forecast from Warning with explicit yellow-versus-red map and event-panel treatment. The selected-location P/S card now follows the explicitly selected UI language, translates its Japanese ground-proxy station for English/Czech display, and uses a compact layout without dropping calculation disclosures. Expanding predicted intensities during an active EEW no longer leaves an invisible inline alert banner consuming a large empty block above the destination card.
- Keeps locally modelled Shindo 0 regions off the coloured map overlay so unforecasted Japan retains its normal neutral land colour. When DM-D.S.S supplies an official nationwide maximum without regional values, the ignored local engine now minimally aligns its continuous spatial estimate to that official category, preventing the map maximum from contradicting the provider maximum while preserving the locally calculated distribution.
- Prevents a partially populated official forecast from suppressing all locally calculated missing regions or blocking the selected-location estimate merely because a different JMA area was included.
- Recognizes P2PQuake packet `_id` values for live deduplication, including crowd-signal aggregates.
- Coalesces DM-D.S.S JSON pings so only the newest queued `pingId` is answered. Socket failures now retain the network transport, socket lifetime, current-versus-stale listener state, and planned reconnect delay in the on-screen and exported delivery diagnostics.
- Orders archived EEW, felt-report, and official earthquake replay frames by their source bulletin time instead of database insertion time. Cumulative felt counts cannot move backwards when recovery packets arrive out of order, propagate from the active EEW to the matching confirmed incident, remain visible on historical felt frames, and use a dedicated unclipped line in the live Recent earthquakes list.
- Adds the final P2PQuake felt-report count to each matching historical-event catalogue row and restores the same final count into Recent earthquakes from live EEW claims, the always-on recent-report cache, or the opt-in raw archive. A preliminary EEW claim survives a small official-origin revision; unmatched aggregates still remain diagnostics-only and never create standalone events.
- Renders active EEW P/S-wave rings and locally calculated intensity areas when a valid offshore source lies just outside the strict bundled Japan-map extent. The existing map bounds remain unchanged; camera focus clamps to the nearest representable edge at a regional minimum zoom instead of falling back to the all-Japan view. Confirmed earthquake reports retain their previous Japan-only focus policy.
- Bumps the Android development build to `0.10.1-dev.13` (`versionCode` 232) and updates the README release marker.
