...
- Keeps every QuakeDeck-generated ground-motion calculation in the deliberately Git-ignored `LocalEewForecastEngine.kt`. The single public repository remains buildable: `FULL` includes the local implementation; `LITE` keeps official JMA, DM-D.S.S, and P2PQuake data without local intensity estimates, P/S rings, or destination countdowns. The README identifies the omitted path and legal publication boundary.
- Adds permanent FULL/LITE edition badges and a generated compilation marker that invalidates stale engine bytecode after source removal or edition changes. The build supports a direct local Boolean switch and a Gradle-property override.
- Adds separately labelled local intensity estimates using JMA's published Mj/Mw conversion, finite-source distance bounds, PGV, AVS30 amplification, and instrumental-intensity method. A derived NIED J-SHIS V4 lookup covers 4,243 valid station meshes across 188 reporting areas; 83 uncovered coordinates and 34 invalid zero-AVS records are explicitly excluded.
- Replaces fixed P/S velocities with interpolation of the official JMA2001 travel-time table. Estimates below the published intensity-validation range are disclosed; assumed-source PLUM and source depths greater than 150 km suppress local intensity calculation.
- Uses an official-authoritative hybrid: received regional values remain unchanged, local estimates fill missing areas, and official regional categories plus the nationwide maximum calibrate the local field. Where official regions exist, omitted areas are constrained below Shindo 4. Selected-location estimates and notification decisions use an official local value first, then an eligible local estimate.
- Preserves source accuracy, magnitude unit, region code, PLUM, and Warning metadata. Map shading, regional rows, selected-location notifications, diagnostics, cold-start restoration, and Sandbox understand the separate local result and its provenance.

### Historical replay and felt reports

- Archives accepted, deduplicated DM-D.S.S VXSE44/VXSE45 Forecast revisions and P2PQuake code-556 Warnings through one source-neutral EEW frame format, preserving source, alert level, revision, official areas, and intensity state.
- Replays EEW, attached cumulative felt-report snapshots, and official reports through the existing Previous/Next controls, ordered by bulletin time rather than database insertion time.
- Colours historical EEW areas using the same official-first/local hybrid calculation as live. Each revision uses its own archived source inputs; later EEW or confirmed-report values cannot leak backward. Historical mode remains static: no P/S rings, destination countdowns, live alerts, or automatic refits between steps.
- Retains calculation inputs and official region metadata in new archive frames. Older frames remain readable and use the inputs they retained; metadata already omitted by older builds cannot be recovered. FULL recomputes estimates with the installed engine/resources, while LITE retains official-only replay.
- Adds opt-in P2PQuake code-9611 felt-report counters under Data & connection. Informative aggregates attach to a matching active P2PQuake/DM-D.S.S EEW or the nearest confirmed earthquake within the association window; unmatched or insufficient-confidence aggregates stay diagnostics-only and never create felt-only events.
- Keeps cumulative felt counts monotonic despite out-of-order recovery, transfers them from an EEW to its matching confirmed earthquake, and retains the claim across small official-origin revisions.
- Shows attached counts in replay and final counts in the historical-event catalogue and Recent earthquakes. Recent counts can be restored from live claims, the recent-report cache, or the opt-in raw archive.

### Delivery diagnostics and connections

- Adds a human-readable newest-20 packet timeline combining DM-D.S.S and P2PQuake traffic. Export retains source-labelled, credential-redacted raw payloads.
- Excludes routine ping/pong and P2PQuake code-555 peer-count broadcasts. Retention trims old entries only after both thresholds are reached: 200 entries and 2 MB.
- Recognizes P2PQuake packet `_id` values for deduplication, including felt aggregates.
- Answers only the newest queued DM-D.S.S `pingId`. Failure diagnostics retain network transport, socket lifetime, current-versus-stale listener state, and reconnect delay.

### Notification and map fixes

- Keeps regular P2PQuake reports tied to their own confirmed JMA data during an active DM-D.S.S EEW. Explicit earthquake-versus-EEW notification destinations prevent forecast intensity leaking into a regular notification or reopening an expired EEW.
- Distinguishes Forecast in yellow from Warning in red. The compact destination card follows the selected UI language and translates its ground-proxy station; expanding predicted intensities no longer leaves an invisible alert-banner gap.
- Leaves modelled Shindo 0 areas unpainted, preserving neutral land colours. A partial official forecast no longer suppresses all local supplements or blocks a destination estimate merely because another area has an official value.
- Renders active EEW rings and local intensity areas for valid offshore sources just beyond the bundled map extent. Regional focus clamps to the nearest representable edge without expanding map bounds; confirmed reports retain their previous Japan-only focus policy.
- Carries P2PQuake's non-predictive fallback expiry through runtime state and notification restoration, preventing an indefinitely retained active warning in LITE.

### Build and release

- Finalizes `0.10.1` with Android `versionCode` 233.
- Updates compileSdk to 37, Core to 1.19.0, Activity Compose to 1.13.0, OkHttp to 5.5.0, and JSON-java tests to 20260814; removes the duplicate Android-test Compose BOM. targetSdk remains 36 pending deliberate Android 17 behavior validation.
- DM-D.S.S paths without observed live coverage remain pending live-event validation; finalizing the release does not label those paths production-verified.
